...
class NumericStringParser:
    ...
    def __init__(self, evaluator):
        ...
        independent_atom = pp.Or([pp.Literal(atom.name)
                                  for atom in self.evaluator.independent_atoms]) \
            .setParseAction(lambda toks: Atom(toks[0]))

        dependent_atom = (pp.Or([pp.Literal(atom.name)
                                 for atom in self.evaluator.dependent_atoms]) +
